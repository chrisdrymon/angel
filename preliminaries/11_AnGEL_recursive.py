...
# I'm going to take the output of LSTM 2, and keep the annotation that the tagger is most confident about. Then I'll
# set that aspect's tensor to a one-hot state and keep everything else the same value that LSTM 2 gave it. After that,
# each token within a 7 token window to the left and right of the token with the one-hotted feature is ran back through
# the DNN with its adjusted one-hot value(s). After the DNN, probably reset the one-hot and then run it through
# LSTM2. Continue doing this until every tag is made.
# To make the process faster, if the window around the second most confident tag does not touch the window of the first
# most confident tag, then go ahead and accept that tag as well and one-hot its aspect. Continue doing this until
# a tag's window would have overlapped with another. Then all the now-effected tokens within all the windows are fed
# through the DNN and LSTM all at once. And the process is repeated.
