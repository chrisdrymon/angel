...
for test_file in sorted(os.listdir(corpora))[10:]:
        ...
        for i, token in enumerate(split_text):
            for aspect in morphs:
                if aspect.predicted_tags3[i] == aspect.correct_tags[i]:
                    ...
                    aspect.total_correct += 1
        print(f'Sentence {sentence["id"]}: {split_text}')
        for aspect in morphs:
            print(f'{aspect.title} correct: {aspect.total_correct}/{total_tokens} = '
                  f'{aspect.total_correct/total_tokens:.02%}')
